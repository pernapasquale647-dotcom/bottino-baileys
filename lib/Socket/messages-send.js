...
const make_anti_ban_1 = require("../Utils/make-anti-ban")
...
const makeMessagesSocket = (config) => {
    ...
    const antiBan = (0, make_anti_ban_1.makeAntiBan)(config)
    const sendMessageQueue = antiBan.enqueue
    ...
    const sendTypingIndicator = async (toJid) => {
        try {
            ...
            logger.debug({ err: e }, 'anti-ban: failed to send typing indicator')
        }
    }

    const sendViaQueue = (jid, fn) => sendMessageQueue(jid, async () => {
        const humanize = config.humanizeMessages !== false
        const antiBanOpts = config.antiBan || {}
        if (humanize) {
            const minDelay = typeof antiBanOpts.minDelayMs === 'number' ? antiBanOpts.minDelayMs : 800
            const maxDelay = typeof antiBanOpts.maxDelayMs === 'number' ? antiBanOpts.maxDelayMs : 2500
            if (antiBanOpts.typingIndicator !== false && !WABinary_1.isJidNewsletter(jid)) {
                await sendTypingIndicator(jid)
            }
            await Utils_1.delay(antiBan.randomBetween(minDelay, maxDelay))
        }
        return fn()
    })
