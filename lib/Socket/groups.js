...
const makeGroupsSocket = (config) => {
    ...
    const groupFetchAllParticipating = async () => {
        ...
        sock.ev.emit('groups.update', groups);
        for (const meta of groups) {
            CacheManager.set('groupMetadataCache', meta.id, meta, perfConfig.cache.groupMetadataCache.ttl);
        }
        return data;
    };
    ...
    return {
        ...
        groupRequestParticipantsList: async (jid) => {
            ...
            return participants.map(v => {
                ...
                let resolvedJid = attrs.phone_number || (0, WABinary_1.lidToJid)(rawJid) || rawJid;
                if (!resolvedJid.includes('@')) {
                    resolvedJid = resolvedJid.replace(/^\+/, '') + '@s.whatsapp.net';
                } else {
                    resolvedJid = resolvedJid.replace(/^\+/, '');
                }
                return {
                    ...attrs,
                    jid: resolvedJid,
        ...
        groupParticipantsUpdate: async (jid, participants, action) => {
            ...
            let metadata = null;
            try {
                metadata = await groupMetadata(jid);
                if (metadata && metadata.addressingMode === 'lid') {
                    isLidGroup = true;
                }
            } catch (e) { }

            participants = participants.map(p => {
                let resolved = resolveJid(p);
                if (isLidGroup && resolved.endsWith('@s.whatsapp.net')) {
                    const WABinary_1 = require("../WABinary");
                    const lidCache = WABinary_1.sharedLidPhoneCache;
                    if (metadata && metadata.participants) {
                        const member = metadata.participants.find(m => m.jid === resolved || m.id === resolved);
                        if (member && member.lid) return member.lid;
                        if (member && member.id && member.id.endsWith('@lid')) return member.id;
                    }
                    if (lidCache && typeof lidCache.getLidForPhone === 'function') {
                        const lid = lidCache.getLidForPhone(resolved);
                        if (lid) return lid;
                    }
                }
                return resolved;
...
const extractGroupMetadata = (result) => {
    ...
    if (!group) {
        const errorNode = (0, WABinary_1.getBinaryNodeChild)(result, 'error');
        if (errorNode) {
    ...
    const metadata = {
        ...
        participants: (0, WABinary_1.getBinaryNodeChildren)(group, 'participant').map(({ attrs }) => {
            // jid = phone JID (@s.whatsapp.net), resolved from phone_number attr, lidToJid(), or the raw jid
            const jid = attrs.phone_number
                ? (attrs.phone_number.includes('@') ? attrs.phone_number : attrs.phone_number + '@s.whatsapp.net')
                : (0, WABinary_1.lidToJid)(attrs.jid);
            // lid = raw LID (@lid), if the original jid was a LID or a separate lid attr is present
            const lid = attrs.lid || ((0, WABinary_1.isLid)(attrs.jid) ? attrs.jid : undefined);
            // Populate the shared LID<->phone cache so future resolution is instant
            if (jid && lid && jid !== lid && (0, WABinary_1.isLid)(lid) && !(0, WABinary_1.isLid)(jid)) {
                WABinary_1.sharedLidPhoneCache.set(lid, jid);
            }
            // Use resolved phone JID as `id` so all downstream code operates on real JIDs.
            // If resolution failed (LID still unresolvable), fall back to raw attrs.jid.
            const resolvedId = (jid && !(0, WABinary_1.isLid)(jid)) ? jid : attrs.jid;
            return {
                id: resolvedId,
                jid: resolvedId,
                lid,
