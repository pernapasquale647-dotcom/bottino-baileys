...
const replacePhoneWithLid = (obj, phoneUser, lidUser) => {
    if (!obj || typeof obj !== 'object' || obj === null) {
        return;
    }
    for (const key in obj) {
        const val = obj[key];
        if (typeof val === 'string' && val.includes(`@${phoneUser}`)) {
            obj[key] = val.replace(new RegExp(`@${phoneUser}(?!\\d)`, 'g'), `@${lidUser}`);
        }
        else if (typeof val === 'object' && val !== null) {
            replacePhoneWithLid(val, phoneUser, lidUser);
        }
    }
};
const normalizeMentionedJids = (mentions, messageContent) => {
    ...
    return mentions
        .map(jid => {
            ...
            // Convert phone to LID if possible
            if (out && !(0, WABinary_1.isLid)(out)) {
                const lid = WABinary_1.sharedLidPhoneCache.getLidForPhone(out);
                if (lid) {
                    if (messageContent) {
                        const phoneUser = (0, WABinary_1.jidDecode)(out).user;
                        const lidUser = (0, WABinary_1.jidDecode)(lid).user;
                        if (phoneUser && lidUser) {
                            replacePhoneWithLid(messageContent, phoneUser, lidUser);
                        }
                    }
                    out = lid;
                }
...
const generateWAMessageContent = async (message, options) => {
        ...
        if (thumbUrl && (!currentThumb || currentThumbLen < 2000)) {
            try {
                const response = await (0, axios_1.default)({
                    ...
                    timeout: 7000,
                    headers: {
                        'User-Agent': 'WhatsApp/2.26.13.73 Android/16 Device/Redmi_Note_13_Pro_5G'
                    }
                });
                const downloaded = Buffer.isBuffer(response?.data) ? response.data : Buffer.from(response?.data || []);
                if (downloaded.length) {
                    const { buffer } = await (0, messages_media_1.extractImageThumb)(downloaded, 192, 60);
                ...
                options.logger?.warn({ err: error?.message, url: thumbUrl }, 'failed to download externalAdReply thumbnail');
            }
        }
        const adThumbBuf = externalAdReply.thumbnail;
        const hasThumbBytes = !!(adThumbBuf && (adThumbBuf.length || adThumbBuf.byteLength));
        const isPrivateThumbUrl = (u) => typeof u === 'string' && /(?:^|[\/\.])[a-z0-9-]*\.?whatsapp\.net\//i.test(u);
        if (hasThumbBytes) {
            if (isPrivateThumbUrl(externalAdReply.thumbnailUrl)) {
                delete externalAdReply.thumbnailUrl;
            }
            if (isPrivateThumbUrl(externalAdReply.originalImageUrl)) {
                delete externalAdReply.originalImageUrl;
            }
        }
        if (externalAdReply.mediaType == null) {
            externalAdReply.mediaType = hasThumbBytes ? 1 : 0;
        }
        if (externalAdReply.showAdAttribution === undefined) {
            externalAdReply.showAdAttribution = false;
        }
        if (externalAdReply.renderLargerThumbnail === undefined) {
            externalAdReply.renderLargerThumbnail = hasThumbBytes;
        }
    ...
    if ('text' in message) {
        ...
        const { text, code, language = 'javascript', botJid = DEFAULT_BOT_JID } = message.richResponse;
        const sections = [];
        ...
        if (code) {
            const message_composer_1 = require('./message-composer');
            sections.push({
                view_model: {
                    primitive: {
                        language,
                        code_blocks: message_composer_1.tokenizeCode(String(code), language),
        ...
        if (sections.length === 0) {
            throw new boom_1.Boom('richResponse requires at least one of text or code');
        }
    ...
    if ('shop' in message && !!message.shop) {
        ...
        if ('contextInfo' in message && !!message.contextInfo) {
            interactiveMessage.contextInfo = { ...message.contextInfo };
            if (Array.isArray(interactiveMessage.contextInfo.mentionedJid) && interactiveMessage.contextInfo.mentionedJid.length) {
                interactiveMessage.contextInfo.mentionedJid = normalizeMentionedJids(interactiveMessage.contextInfo.mentionedJid, interactiveMessage);
            }
        }
        if ('mentions' in message && !!message.mentions) {
            interactiveMessage.contextInfo = { mentionedJid: normalizeMentionedJids(message.mentions, interactiveMessage) };
        }
        m = { interactiveMessage };
    }
    ...
    if (('viewOnce' in message && !!message.viewOnce) && !hasViewOnceAlready) {
    ...
    if ('mentions' in message && ((_o = message.mentions) === null || _o === void 0 ? void 0 : _o.length)) {
        ...
        m[messageType].contextInfo.mentionedJid = normalizeMentionedJids(message.mentions, m);
    }
    if ('edit' in message) {
        m = {
            protocolMessage: {
    ...
    if ('contextInfo' in message && !!message.contextInfo) {
        ...
        if (Array.isArray(contextInfo.mentionedJid) && contextInfo.mentionedJid.length) {
            contextInfo.mentionedJid = normalizeMentionedJids(contextInfo.mentionedJid, m);
        }
