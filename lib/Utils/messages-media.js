...
async function uploadFile(buffer, logger) {
    const fileTypeLib = await Promise.resolve().then(() => __importStar(require('file-type')));
    const detectFn = fileTypeLib.fileTypeFromBuffer || fileTypeLib.fromBuffer || (fileTypeLib.default && (fileTypeLib.default.fileTypeFromBuffer || fileTypeLib.default.fromBuffer));
    if (!detectFn) {
        throw new Error("file-type library is missing fromBuffer and fileTypeFromBuffer functions.");
    }
    const fileType = await detectFn(buffer);
    if (!fileType)
