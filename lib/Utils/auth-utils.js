...
function makeCacheableSignalKeyStore(store, logger, _cache) {
    ...
    const cache = _cache || new node_cache_1.default({
        stdTTL: 0, // never expire — write-through cache, keys are session-critical
        useClones: false,
        deleteOnExpire: true,
    });
    ...
    return {
        async get(type, ids) {
            ...
            for (const id of ids) {
                ...
                if (item === DELETED) {
                    // key was explicitly deleted, don't re-fetch from disk
                }
        ...
        async set(data) {
            ...
            for (const type in data) {
                for (const id in data[type]) {
                    ...
                    cache.set(getUniqueId(type, id), val === null || val === undefined ? DELETED : val);
                    keys += 1;
                }
            }
