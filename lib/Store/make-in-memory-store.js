...
exports.default = (config) => {
    ...
    const MAX_MESSAGES_PER_CHAT = config.maxMessagesPerChat || 25;
    const MAX_CHATS_WITH_MESSAGES = config.maxChatsWithMessages || 150;
    const MAX_CONTACTS = config.maxContacts || 1000;
    const MAX_GROUP_METADATA = config.maxGroupMetadata || 100;
    const MAX_PRESENCES = config.maxPresences || 50;
    ...
    const assertMessageList = (jid) => {
        if (!messages[jid]) {
            ...
            if (chatKeys.length >= MAX_CHATS_WITH_MESSAGES) {
                const toRemove = chatKeys.slice(0, chatKeys.length - MAX_CHATS_WITH_MESSAGES + 1);
    ...
    const bind = (ev) => {
        ...
        ev.on('messaging-history.set', ({ chats: newChats, contacts: newContacts, messages: newMessages, isLatest, syncType }) => {
            if (syncType === WAProto_1.proto.HistorySync.HistorySyncType.ON_DEMAND) {
                ...
                if (performance_config_1.getPerformanceConfig().performance.enableMetrics) {
                    if (global.gc) {
                        try {
                            global.gc();
                        } catch(e) {}
        ...
        ev.on('presence.update', ({ id, presences: update }) => {
            ...
            if (presenceKeys.length > MAX_PRESENCES) {
                const toRemove = presenceKeys.slice(0, presenceKeys.length - MAX_PRESENCES);
                for (const k of toRemove) {
                    delete presences[k];
                }
            }
        ...
        ev.on('groups.update', updates => {
            for (const update of updates) {
                ...
                if (groupMetadata[id]) {
                    ...
                    const gmKeys = Object.keys(groupMetadata);
                    if (gmKeys.length >= MAX_GROUP_METADATA) {
                        const toRemove = gmKeys.slice(0, gmKeys.length - MAX_GROUP_METADATA + 1);
                        for (const k of toRemove) {
                            delete groupMetadata[k];
                        }
                    }
